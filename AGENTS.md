...
- `pages/` contains file-based Nuxt routes. Dynamic routes use bracket notation, for example `pages/article/[link].vue`.
- `components/` holds reusable UI. Feature components live in `components/article/`, base UI in `components/ui/`.
- `stores/` contains Pinia stores such as `stores/articles.js`.
...
- Component filenames are PascalCase (`Header.vue`, `ArticleCard.vue`). Base UI components use the `Ui` prefix (
  `UiButton.vue`).
...
- If adding tests, document the location (for example `tests/` or `components/**/__tests__`) and add a script in
  `package.json` (for example `"test": "vitest"`).

## Commit & Pull Request Guidelines

- Recent commits use short, lowercase, action-oriented summaries (for example `add hover in card`).
...
- Do not commit secrets. If environment variables are introduced, add an `.env.example` and document required keys.
